...
Sapiens2 프로젝트에 Apple Silicon MPS용 RTMDet-nano + RTMPose-M 실행 환경을 추가한다. 러닝 영상 프레임에서 얼굴 상세점 없이 COCO 17개 신체 관절을 추론하고, 기존 Sapiens2 환경과 출력물을 손상시키지 않는다.
...
Sapiens2의 `.venv-sapiens2`와 별도로 `.venv-rtmpose`를 둔다. 외부 저장소는 `Sapiens2/mmpose`와 `Sapiens2/mmdetection`에 고정 커밋으로 배치해 공식 프로젝트 구조와 재현성을 보존한다.
...
3. 모델 다운로드 스크립트는 OpenMMLab의 공식 URL에서 RTMDet-nano와 RTMPose-M/S 가중치를 `models/rtmpose/` 아래에 저장한다.
4. 실행 스크립트는 이미지 디렉터리를 받아 MPS에서 top-down pose inference를 수행하고, 시각화와 `pose_predictions.json`을 `outputs/rtmpose/pose/`에 작성한다. JSON은 image path, bbox, bbox score, COCO 17 keypoints, keypoint scores를 포함한다.
